...
int main (int argc, char **argv)
{
    ...
    // if((node_hp = gethostbyname(config[0].ip_port.ip_address)) == NULL){
    ...
    // }
    // char buff[5000];
    // char postdata[5000] = "param1=value1&param2=value";  /* Change param and value here */

    // sprintf(buff,"POST /status/packet_lost HTTP1.1\r\nAccept: */*\r\nAccept-Language: en-us\r\nContent-Type: application/x-www-form-urlencoded\r\nAccept-Encoding: gzip,deflate\r\nContent-Length: %d\r\nConnection: keep-alive\r\n\r\n%s",strlen(postdata),postdata);
    ...
    while (1) {
        //char *header = "POST /status/no_loss HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\nContent-Length: 1024\r\nConnection: keep-alive\r\n\r\nparam1=lost&param2=forward\n";
        //send(node_fd,header,strlen(header),0);
        while (1) {
            ...
            if ((n = recvfrom (sd1, temp, sizeof(*temp), 0, (struct sockaddr *)&sender, (unsigned int *)&sender_len)) < 0) {
                ...
                if ((temp->PacketType == 1 || temp->PacketType == 4) && (temp->WindowSize != 0)) { // SYN OR DATA OR EOT
                    ...
                    if ((noise < 101 && noise > 89)) {
                        ...
                        // char *header = "POST /status/packet_lost HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\nContent-Length: 1024\r\nConnection: keep-alive\r\n\r\nparam1=lost&param2=forward\n";
                        // send(node_fd,header,strlen(header),0);
                        //sleep(1.5);
            ...
            if ((n = recvfrom (sd2, temp, sizeof(*temp), 0, (struct sockaddr *)&receiver, (unsigned int *)&receiver_len)) < 0) {
                ...
                if (temp->PacketType == 2 || temp->PacketType == 3) { // SYNACK OR ACK
                    ...
                    if ((temp->re == 1 || temp->last == 1) && noise != 100) {
                        ...
                        // char *header = "POST /status/ack_loss HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\nContent-Length: 1024\r\nConnection: keep-alive\r\n\r\nparam1=lost&param2=forward\n";
                        // send(node_fd,header,strlen(header),0);
                        sleep(1.5);
                    }
                } else {
